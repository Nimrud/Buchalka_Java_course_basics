...
public class Main {
    ...
    //   - A constructor that takes a String (the phone number) and initialises myNumber
    //   and instantiates myContacts.
    ...
    //      updated with the new contact) and returns a boolean. Returns true if the contact exists
    //      and was updated successfully, or false if the contact doesn't exists.
    ...
    //      The returned value is it's position in the ArrayList, it will either be -1
    ...
    //      >  createContact(), has two parameters of type String (the persons name
    //      and phone number) and returns an instance of Contact.
    //      This is the only method that is static.
    //
    // TIP: In MobilePhone, use findContact() in the other methods
    // (except printContacts()) to check if it exists before proceeding.
