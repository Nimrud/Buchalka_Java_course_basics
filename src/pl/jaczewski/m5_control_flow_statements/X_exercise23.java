...
public class X_exercise23 {
    ...
    public static void numberToWords (int number) {
        if (number < 0) {
            ...
            int basereversedValue = reversedValue;

            int digit;
            while (reversedValue > 0) {
                digit = reversedValue % 10;
                switch (digit) {
                    case 0:
                        System.out.println("Zero");
                        break;
            ...
            if (getDigitCount(basereversedValue) != getDigitCount(number)){
                int discrepancy = (getDigitCount(number) - getDigitCount(basereversedValue));
                while (discrepancy > 0) {
                    System.out.println("Zero");
                    discrepancy--;
                }
